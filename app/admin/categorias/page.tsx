...
export default function AdminCategoriasPage() {
    ...
    const [form, setForm] = useState({ nombre: '', icono: '', orden: '1' });
    ...
    const abrirCrear = () => {
        setEditando(null);
        setForm({ nombre: '', icono: '', orden: String(categorias.length + 1) });
        setModalAbierto(true);
    };

    const abrirEditar = (cat: Categoria) => {
        setEditando(cat);
        setForm({ nombre: cat.nombre, icono: cat.icono ?? '', orden: String(cat.orden) });
        setModalAbierto(true);
    };
    ...
    const handleGuardar = async () => {
        ...
        setGuardando(true);
        const datos = { nombre: form.nombre, icono: form.icono || null, orden: parseInt(form.orden) || 1 };
